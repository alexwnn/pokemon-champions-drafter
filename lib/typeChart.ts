...
export function singleEffectiveness(atk: TypeName, def: TypeName): number {
...
export function effectiveness(
  ...
  return defendingTypes.reduce(
    (acc, d) => acc * singleEffectiveness(attackingType, d),
    1,
  );
}

export function bestEffectivenessFromTypes(
  attackingTypes: TypeName[],
  defendingTypes: TypeName[],
): number {
  let best = 0;
  for (const atk of attackingTypes) {
    const m = effectiveness(atk, defendingTypes);
    if (m > best) best = m;
  }
  return best;
}

export function worstIncoming(
  defendingTypes: TypeName[],
  threatTypes: TypeName[],
): number {
  let worst = 0;
  for (const t of threatTypes) {
    const m = effectiveness(t, defendingTypes);
    if (m > worst) worst = m;
  }
  return worst;
}
